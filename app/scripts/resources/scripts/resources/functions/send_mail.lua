...
if (email_queue_enabled == 'true') then
	function send_mail(headers, email_from, email_address, email_message, email_file)
		...
		local domain_uuid = headers["X-IungoPBX-Domain-UUID"];
		local domain_name = headers["X-IungoPBX-Domain-Name"];
		local email_type = headers["X-IungoPBX-Email-Type"] or 'info';
		local call_uuid = headers["X-IungoPBX-Call-UUID"];
		local local_after_email = headers["X-IungoPBX-local_after_email"] or '';
	...
	--local headers = {
	--	["X-IungoPBX-Domain-UUID"] = '2d171c4c-b237-49ca-9d76-9cffc1618fa7';
	--	["X-IungoPBX-Domain-Name"] = 'domain.com';
	--	["X-IungoPBX-Email-Type"]	= 'voicemail';
	--}
	...
	if freeswitch then
		function send_mail(headers, from, address, message, file)
			...
			local domain_name = headers["X-IungoPBX-Domain-Name"]
			local email_type = headers["X-IungoPBX-Email-Type"] or 'info'
			local call_uuid = headers["X-IungoPBX-Email-Call-UUID"]
			local db = dbh or Database.new('system')
			local settings = Settings.new(db, domain_name, domain_uuid)
			local xheaders = "{"
