...
//define the process id file
	$pid_file = "/var/run/iungopbx/email_send".".".$email_queue_uuid.".pid";
...
//make sure the /var/run/iungopbx directory exists
	if (!file_exists('/var/run/iungopbx')) {
		$result = mkdir('/var/run/iungopbx', 0777, true);
		if (!$result) {
			die('Failed to create /var/run/iungopbx');
		}
	}
...
//send the email
	if ($sent) {
		...
		*/
		...
		/*
		$call_uuid = $headers["X-IungoPBX-Call-UUID"];
		if ($resend == true) {
			...
			if (!isset($email_log_uuid)) {
				//build insert array
					...
					$array['email_logs'][0]['domain_uuid'] = $headers["X-IungoPBX-Domain-UUID"];
					$array['email_logs'][0]['sent_date'] = 'now()';
					$array['email_logs'][0]['type'] = $headers["X-IungoPBX-Email-Type"];
