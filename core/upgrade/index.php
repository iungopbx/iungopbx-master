...
//process the http post
	if (sizeof($_POST) > 0) {
		...
		//run source update
		if ($action["upgrade_source"] && permission_exists("upgrade_source") && !is_dir("/usr/share/examples/iungopbx")) {
			$cwd = getcwd();
...
//show the content
	...
	if (permission_exists("upgrade_source") && !is_dir("/usr/share/examples/iungopbx") && is_writeable($_SERVER["PROJECT_ROOT"]."/.git")) {
		...
		if (($branch_return_value == 0) && ($commit_return_value == 0)) {
			...
			echo "<a href='https://github.com/iungopbx/iungopbx/compare/";
