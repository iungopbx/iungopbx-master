...
//add the document root to the include path
	...
	if (is_array($config_glob) && count($config_glob) > 0) {
		...
		$config_php_glob = glob("{/usr/local/etc,/etc}/iungopbx/config.php", GLOB_BRACE);
		...
		if (stristr(PHP_OS, 'BSD')) {
			...
			$document_root = '/usr/local/www/iungopbx';
		...
		if (stristr(PHP_OS, 'Linux')) {
			...
			$config_file = $config_path.'/config.conf';
			$document_root = '/var/www/iungopbx';
		...
		$conf .= "cache.location = /var/cache/iungopbx\n";
