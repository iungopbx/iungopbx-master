...
//if the config.php exists create the config.conf file
	if (!$config_exists) {
		if (file_exists("/usr/local/etc/iungopbx/config.php")) {
			//bsd
			$config_path = "/usr/local/etc/iungopbx";
		}
		elseif (file_exists("/etc/iungopbx/config.php")) {
			//linux
			$config_path = "/etc/iungopbx";
		}
...
//process and save the data
	if (count($_POST) > 0) {
		...
		if ($_REQUEST["step"] == "install") {
			...
			//set the include path
			$config_glob = glob("{/usr/local/etc,/etc}/iungopbx/config.conf", GLOB_BRACE);
			$conf = parse_ini_file($config_glob[0]);
			set_include_path($conf['document.root']);

			//add the database schema
			$output = shell_exec('cd '.$_SERVER["DOCUMENT_ROOT"].' && php /var/www/iungopbx/core/upgrade/upgrade_schema.php');
			...
			//app defaults
			$output = shell_exec('cd '.$_SERVER["DOCUMENT_ROOT"].' && php /var/www/iungopbx/core/upgrade/upgrade_domains.php');
...
//assign default values to the template
	...
	$view->assign("database_name", "iungopbx");
	$view->assign("database_username", "iungopbx");
